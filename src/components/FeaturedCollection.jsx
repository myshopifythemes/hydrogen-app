...
export default function FeaturedCollection({collection}) {
  return collection ? (
    <div className="shadow-xl rounded-xl grid grid-cols-1 lg:grid-cols-2 items-center bg-white overflow-hidden mb-10">
      {collection.image ? (
        <Image width="622" height="465" data={collection.image} alt= {collection.title} />
      ) : null}
      <div className="px-10 py-10 lg:py-0">
        <h2 className="text-gray-700 text-3xl font-bold mb-5">
          {collection.title}
        </h2>
        <p className="line-clamp-3 text-lg text-gray-500 mb-6">{collection.description}</p>
        <Link
